...
class MainListViewModelTest {
    private val getRandomUsersUseCaseMock: GetRandomUsersUseCase = mock()
    ...
    fun setUp() {
        ...
        viewModel =
            MainListViewModel(getRandomUsersUseCase = getRandomUsersUseCaseMock)
    }
    ...
    fun `getUsers ok`() = runTest {
        whenever(getRandomUsersUseCaseMock.run()).thenReturn(
            flowOf(
                PagingData.from(
